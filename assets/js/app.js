   // --- CONSTANTS AND MAPPING DATA ---
        ...
        // --- 7️⃣ Sandhi Analysis Logic (FIXED) ---
        ...
        const readableSandhiType = {
            ...
            'yan_y': 'Yaṇ Sandhi (vowel modification)',
            'yan_v': 'Yaṇ Sandhi (vowel modification)',
            'unknown': 'N/A'
        };

        /**
         * Performs simplified reverse sandhi analysis on a single IAST token.
         */
        function performSandhiSegmentation(iastText) {
            ...
            // --- 7️⃣ FIX: Guna Sandhi (Medial 'e' or 'o') ---
            // Guna 'e' -> a/ā + i/ī (e.g., maheśvara -> mahā + īśvara)
            try {
                // 7️⃣ FIX: Regex now splits at 'e' and proposes 'i/ī' for the second part.
                const gunaERegex = new RegExp(`^(.*)e(.+)$`); 
                const gunaEMatch = text.match(gunaERegex);
                if (gunaEMatch && !found) {
                    const [, p1, p2] = gunaEMatch;
                    const proposal = `${p1}a/ā + i/ī${p2}`; // FIX
            ...
            try {
                // 7️⃣ FIX: Regex now splits at 'o' and proposes 'u/ū' for the second part.
                const gunaORegex = new RegExp(`^(.*)o(.+)$`);
                const gunaOMatch = text.match(gunaORegex);
                if (gunaOMatch && !found) {
                    const [, p1, p2] = gunaOMatch;
                    const proposal = `${p1}a/ā + u/ū${p2}`; // FIX
            ...
            try {
                // 7️⃣ FIX: Modified regex slightly to ensure second part starts with correct vowel class
                const dirghaARegex = new RegExp(`^(.*)ā([${VOWELS_A}].*)$`); 
                const dirghaAMatch = text.match(dirghaARegex);
                 if (dirghaAMatch && !found) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            analyzeSandhiButton.addEventListener('click', () => {
                ...
                try {
                    ...
                    if (input.trim()) {
                        const analysis = performSandhiSegmentation(input);
                        ...
                        // Populate and show the log button
                        sandhiLogPre.textContent = analysis.log.join('\n');
                        showSandhiLogBtn.classList.remove('hidden');
                        
                        if (analysis.typeKey !== 'unknown') {
                            showMessage("Sandhi analysis complete.");
                        } else {
            ...
            console.log("--- Running Lipisetu Test Suite (v1.5 - Stable Init) ---");
